...
function drawLine(points, trafficArr) {
    ...
    trafficArr.forEach(seg => {
        ...
        const pl1 = new Tmapv2.Polyline({
            ...
            strokeWeight: 6, iconAnchor: new Tmapv2.Point(16, 16),
            map: map
        });
...
function fetchSpeed() {
    navigator.geolocation.getCurrentPosition(function (position) {
        ...
        .then(weather => {
            // 날씨 기준 감속 판단
            const isBadWeather = parseFloat(weather.pcp) > 2 || parseFloat(weather.sno) > 1;
            const slowFactor = isBadWeather ? 0.8 : 1;
            ...
            fetch(`/speed?lat=${lat}&lon=${lon}`)
                ...
                .then(data => {
                    ...
                    if (data.speed_start && data.speed_end) {
                        const speed1 = (data.speed_start * slowFactor).toFixed(1);
                        const speed2 = (data.speed_end * slowFactor).toFixed(1);
                        display.innerText =
                            `현재 도로: ${data.road}\n` +
                            `시점: ${data.start}, 종점: ${data.end}\n` +
                            `제한속도 (감속 적용): ${speed1} km/h, ${speed2} km/h`;
                    } else {
                        display.innerText = data.message || "제한속도 정보를 찾을 수 없습니다.";
                    }
                })
                .catch(err => {
                    document.getElementById("speedDisplay").innerText = '속도 오류: ' + err;
                });
        })
        .catch(err => {
            document.getElementById("speedDisplay").innerText = '날씨 오류: ' + err;
        });
    });
}
...
// 기존 경로 그리기 함수에 다음 코드도 함께 실행해주세요:
// globalRouteCoords.push(...points); // 각 LineString 처리 안에서
// drawRoute() 끝나고 startLiveNavigation(); 호출
